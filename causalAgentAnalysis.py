...
def A_m_sensors_causal(agent, n_t = 5):
    ...
    input_combinations = [p for p in itertools.product(range(num_input_states), repeat=n_t)]
...
def alpha_ratio_hidden(agent):
    ...
    transitions,counts = get_unique_transitions(agent, return_counts = True, node_ind_pair = None, n_t = 1)
    ...
    for t, trans in enumerate(transitions):
        ...
        transition = pyphi.Transition(network, trans[range(0,agent.n_nodes)],trans[range(agent.n_nodes, 2*agent.n_nodes)],
                        ind_hs, ind_m)

        account = pyphi.actual.account(transition, direction = pyphi.direction.Direction.CAUSE)
        ...
        for d in account:
            ...
            for pur in purviews:
                hidden_purview_idx = np.array([c for c,p in enumerate(pur) if p in agent.hidden_ixs])
                ...
                if len(hidden_purview_idx) == len(pur):
                    ...
                    shapley_values = np.array(compute_shapley_values(d, transition, pur))
                    sum_alpha_hidden_purview = sum_alpha_hidden_purview + sum(shapley_values[hidden_purview_idx])
...
def emptyCausalAnalysis(index_num = 1):
    df = pd.DataFrame(dtype=float, columns = ['E', 'A_4c', 'Phi_max', 'sum_phi_MC', 'sum_phi_full', 'alpha_ratio_hidden'], index = range(index_num))
    return df
...
class Block_World:
	...
	def check_hit(self, game_agent, block):
		agent_position = self.wrapper(range(game_agent.x, game_agent.x+game_agent.pheno_length))
	...
	def _update_agent_position(self, game_agent, world_section):
		agent_rel_sensor_position = self.wrapper([game_agent.x + s for s in game_agent.pheno_sensor_pos])
	...
	def _runTrial(self, trial, agent):
		...
		if (block.type == 'catch' and hit == True) or (block.type == 'avoid' and hit == False):

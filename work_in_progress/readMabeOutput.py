    ...
    for a in list(range(n_agents)):
        for i in list(range(n_trials)):
            for j in list(range(n_transitions + 1)):
                ...
                if j == 0:
                    sensor = np.fromstring(
                        str(data["input_LIST"][ix]), dtype=int, sep=","
                    )[:n_sensors]
                    ...
                    sensor = np.zeros(
                        n_sensors
                    )  # sensors are set to 0 for last transition
                    hidden = np.fromstring(
                        data["hidden_LIST"][ix - 1], dtype=int, sep=","
                    )
                    motor = np.fromstring(
                        data["output_LIST"][ix - 1], dtype=int, sep=","
                    )
                else:
                    sensor = np.fromstring(
                        str(data["input_LIST"][ix]), dtype=int, sep=","

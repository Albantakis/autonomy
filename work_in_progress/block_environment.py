...
class Screen:
    ...
    def saveCurrentScreen(self):
        if len(self.screen_history) == 0:
            ...
            self.screen_history = np.r_[
                self.screen_history, self.screen[np.newaxis, :, :]
            ]
...
class World:
    ...
    def run_fullgame(self, agent, block_patterns):
        ...
        self.history = np.zeros(
            (self.n_trials, self.height, self.height + 1, self.width)
        )

        wins = []
        for trial in range(self.n_trials):
            self.agent, block = self._get_initial_condition(trial)
            self.history[trial, :, :, :], win = self._runGameTrial(
                trial, self.agent, block
            )
            wins.append(win)
    ...
    def _check_win(self, block, agent):
        ...
        win = (
            True
            if (block.type == "catch" and catch)
            or (block.type == "avoid" and not catch)
            else False
        )
        return win
    ...
    def get_final_score(self):
        ...
        for trial in range(self.n_trials):
            ...
            agent.x = self.screen.wrapper(
                agent.x + np.sum(agent.getMotorActivity(trial)[:])
            )

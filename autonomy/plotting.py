...
def plot_animat_brain(agent, state=None, ax=None):
    ...
    colors = np.array([[red_off,green_off,blue_off,grey_off, white],
    ...
    node_colors = [node_colors[i] if node_labels[i] not in isolates else colors[0,4] for i in range(n_nodes)]
    ...
    nx.draw(G, with_labels = True, node_size = 800, node_color = node_colors, edgecolors = '#000000', linewidths=linewidths, pos = flipped_pos, ax = ax)
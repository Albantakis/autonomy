...
def I_SMMI(agent, n_t=1):
    ...
    H_SM = joint_entropy(
        agent, node_ind_pair=[agent.sensor_ixs, agent.motor_ixs], n_t=n_t
    )
    return H_S + H_M - H_SM
...
def I_PRED(agent, n_t=1, noSen=False):
    ...
    if noSen:
        ...
        # Note: sampling is off by n_t states for the joint entropy with n_t > 0
        H_AA = joint_entropy(
            agent, node_ind_pair=[internal_nodes, internal_nodes], n_t=n_t
        )
    else:
...
def IC(agent):
    ...
    H_HHS = joint_entropy(
        agent, node_ind_pair=[internal_nodes, internal_nodes + agent.sensor_ixs], n_t=1
    )
...
def A_m_sensors(agent, n_t=5):
    ...
    states, count = get_unique_multiple_ts(
        agent, return_counts=True, node_ind_array=node_ind_array_joint_1, n_t=n_t
    )
    ...
    states, count = get_unique_multiple_ts(
        agent, return_counts=True, node_ind_array=node_ind_array_cond_1, n_t=n_t - 1
    )
    ...
    states, count = get_unique_multiple_ts(
        agent, return_counts=True, node_ind_array=node_ind_array_joint_2, n_t=n_t
    )
    ...
    states, count = get_unique_multiple_ts(
        agent, return_counts=True, node_ind_array=node_ind_array_cond_2, n_t=n_t - 1
    )
    ...
    A_m = (
        ...
        - entropy(prob_joint_2, base=2)
        + entropy(prob_cond_2, base=2)
    )
    return A_m

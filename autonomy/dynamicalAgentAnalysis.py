...
def number_of_unique_transitions(agent):
    ...
    transitions = get_unique_transitions(
        agent, return_counts=False, node_ind_pair=node_ind_pair, n_t=1
    )
...
def fullDynamicalAnalysis(agent, save_agent=False):
    ...
    dynamical_measures = {
        ...
        "nLZ_tr_space": LZ_complexity(
            agent, dim="space", data=LZ_transient_data(agent)
        ),
    }
